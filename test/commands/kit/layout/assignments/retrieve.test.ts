...
describe('kit:layout:assignments:retrieve', () => {
  ...
  jest.spyOn(Command.prototype, 'getProjectConfig' as any).mockReturnValue(projectConfig);

  const objectNamesFromLayouts = jest.spyOn(Command.prototype, 'objectNamesFromLayouts' as any);

  const findFiles = jest.spyOn(Command.prototype, 'findFiles' as any).mockReturnValue([
  ...
  const getProfileNames = jest.spyOn(Command.prototype, 'getProfileNames' as any).mockImplementation(async () => {
    const names = [];
    for (let i = 1; i <= 12; i++) {
  ...
  const getProfiles = jest.spyOn(Command.prototype, 'getProfiles' as any).mockImplementation(async (names: string[]) =>
    names.map(fullName => ({ fullName, layoutAssignments }))
  );

  const readFile = jest.spyOn(Command.prototype, 'readFile' as any).mockReturnValue({
    Admin: [{ layout: 'Account-Account Layout.layout-meta.xml' }]
  });

  const writeFile = jest.spyOn(Command.prototype, 'writeFile' as any).mockImplementation();
  ...
  t.command(['kit:layout:assignments:retrieve'])
    .it('no arguments', ctx => {
      ...
      expect(findFiles.mock.calls[0][0]).toMatch('force-app/**/*.layout-meta.xml');
      expect(getProfileNames).toHaveBeenCalledTimes(1);
      expect(getProfiles).toHaveBeenCalledTimes(2);
      expect(getProfiles.mock.calls[0][0]).toEqual(['profile1', 'profile2', 'profile3', 'profile4', 'profile5', 'profile6', 'profile7', 'profile8', 'profile9', 'profile10']);
      ...
      expect(writeFile.mock.calls[0]).toEqual(['config/layout-assignments.json', data]);
  ...
  t.command(['kit:layout:assignments:retrieve', '-f', 'config/test.json', '-p', 'Standard', '--merge'])
    .it('-f config/test.json -p Standard --merge', ctx => {
      ...
      expect(writeFile).toHaveBeenCalledTimes(1);
      expect(writeFile.mock.calls[0]).toEqual(['config/test.json', {
        Admin: [{ layout: 'Account-Account Layout.layout-meta.xml' }],
        Standard: [
          {
            layout: 'Account-Account Layout1.layout-meta.xml'
